...
use distributed_frost::{
	create_participant,
	keygen::{Coefficients, Participant},
	FrostInfo,
};
use futures::StreamExt;
use sc_network::{KademliaKey, NetworkService};
use sc_network::{NetworkDHTProvider, NetworkEventStream};
use sc_service::TaskManager;
use sp_application_crypto::{key_types, KeyTypeId};
...
use generic_array::{typenum::UTerm, typenum::B0, typenum::B1, GenericArray};
...
pub const KEY_TYPE_FOR_FROST: KeyTypeId = KeyTypeId(*b"frst");
	...
	TBl: BlockT,
{
	...
	let front_info =
		FrostInfo { totalvalue: FROST_TOTAL_VALIDATORS, thresholdvalue: FROST_THRESHOLD };

	let (party, public_bytes, partycoeffs): (Participant, PublicBytes, Coefficients) =
		create_participant(front_info, 1);
	println!("public key, {:?}", party.public_key().unwrap());
	// let res = keystore.insert(KEY_TYPE_FOR_FROST, suri, public);
	// Task to periodically put the value into the DHT.
	let network_for_put = Arc::clone(&network); // Clone for the put operation
	task_manager
		.spawn_handle()
		.spawn_blocking("AuraPutValueWorker", None, async move {
	...
	task_manager
		.spawn_handle()
		.spawn_blocking("AuraGetValueWorker", None, async move {
			loop {
					...
					sc_network::Event::Dht(sc_network::DhtEvent::ValueFound(records)) => {
						for (found_key, value) in records {
							if &found_key == &dht_key {
								println!(
									"Found value in DHT: {:?}",
									public_bytes.to_vec() == value
								);
							}
						}
					},
					sc_network::Event::Dht(sc_network::DhtEvent::ValueNotFound(queried_key))
						if &queried_key == &dht_key =>
					{
